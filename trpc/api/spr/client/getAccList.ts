...
export const clientAccSprGetList = router({
  ...
  .query(async (opts) => {
    ...
    const query =  format(`
      ...
      a.currency AS client_acc_currency_code,
      ...
      WHERE a.client_id = %1$L;`, input.clientId);
    try {
      const dbClient:PoolClient = await pool.connect();
      const res = await dbClient.query(query);
      dbClient.release();
      const clientAccs  = res.rows.map((item, i, arr) => {
        return {
      ...
      return {list: clientAccs, tc: pool.totalCount, ic: pool.idleCount}
    } catch(err) {
